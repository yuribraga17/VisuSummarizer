...
import ParticleBackground, { FloatingElements, AnimatedGradient } from './components/ParticleBackground';
import { generateSummary } from './utils/summarizer';
import { exportToMarkdown } from './utils/exporter';
import { applyTheme, getThemeColors } from './utils/themeManager';
import { Sparkles, Moon, Sun, Coffee, Zap, BookOpen, Layers, Settings } from 'lucide-react';
...
function App() {
  ...
  const [viewMode, setViewMode] = useState('grid'); // grid, drag, list
  const [showParticles, setShowParticles] = useState(true);

  // Aplica o tema escuro e tema de cores
  useEffect(() => {
    if (darkMode) {
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(() => {
      try {
        ...
        // Aplica cores do tema atual aos tópicos
        const themedSummary = summary.map((topic, index) => ({
          ...topic,
          color: getThemeColors(currentTheme, index)
        }));
        setTopics(themedSummary);
      } catch (error) {
  ...
  const handleExport = () => {
    if (filteredTopics.length > 0) {
      ...
      exportToMarkdown(filteredTopics, title);
    }
  };
  ...
  const handleThemeChange = (newTheme) => {
    ...
    // Reaplica cores aos tópicos existentes
    if (topics.length > 0) {
      const updatedTopics = topics.map((topic, index) => ({
        ...topic,
        color: getThemeColors(newTheme, index)
      }));
      setTopics(updatedTopics);
    }
  };

  const handleTopicsReorder = (reorderedTopics) => {
